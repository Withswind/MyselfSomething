...
gulp.task('stylus-1', function(){
	return gulp.src(paths.stylus[0]).pipe(stylus({
		compress: 1
	})).pipe(gulp.dest('./pro/media/css'));
});

gulp.task('stylus-2', function() {
	return gulp.src(paths.stylus[1]).pipe(stylus({
		compress: 1
	})).pipe(gulp.dest('./pro/em-rem/css'));
});

gulp.task('stylus-3', function(){
	return gulp.src(paths.stylus[2]).pipe(stylus({
		compress : 1
	})).pipe(gulp.dest('./pro/event/css'));
});

gulp.task('stylus-4', function(){
	return gulp.src(paths.stylus[3]).pipe(stylus({
		compress : 1
